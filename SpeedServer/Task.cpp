...
void ResponseGetLicenseInfo(std::shared_ptr<ClientNode> Client)
{
	...
	len = p - buf.data();
	SendToClient(Client, 145, buf.data(), len, Client->ConnID, FE_PLAYER, Client->ConnID, Response);
}
...
m_astSystemTaskPackInfo[].m_shPackMode
	*/
	...
	len = p - buf.data();
	SendToClient(Client, 152, buf.data(), len, Client->ConnID, FE_PLAYER, Client->ConnID, Response);
}
...
void ResponseSkipFreshTask(std::shared_ptr<ClientNode> Client)
{
	...
	len = p - buf.data();
	SendToClient(Client, 183, buf.data(), len, Client->ConnID, FE_PLAYER, Client->ConnID, Response);
}
...
void ResponseGetRandomTask(std::shared_ptr<ClientNode> Client)
{
	...
	*/
	...
	len = p - buf.data();
	SendToClient(Client, 187, buf.data(), len, Client->ConnID, FE_PLAYER, Client->ConnID, Response);
}